...
const FloatingWindow: React.FC<FloatingWindowProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <span style={{ fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-primary)', letterSpacing: '0.2px' }}>{title}</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <button
            title="Desprender ventana (nueva ventana del navegador)"
            onClick={(e) => { e.stopPropagation(); onExternalOpen(); }}
            style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4px', borderRadius: '4px', transition: 'background 0.2s, color 0.2s' }}
