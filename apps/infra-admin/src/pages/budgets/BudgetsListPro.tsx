...
const proToolbarIconStyle: React.CSSProperties = {
  width: '22px',
  height: '22px',
  display: 'inline-flex',
  alignItems: 'center',
...
const proToolbarLabelStyle: React.CSSProperties = {
  fontSize: '0.65rem',
  fontWeight: 'bold',
...
const ProToolbarIcon: React.FC<{ icon: LucideIcon }> = ({ icon: Icon }) => (
  <span style={proToolbarIconStyle} aria-hidden="true">
    <Icon size={14} strokeWidth={2.35} />
  </span>
);

const ProToolbarItem: React.FC<{ label: string; icon: LucideIcon; onClick: () => void }> = ({ label, icon, onClick }) => (
  <div style={proToolbarItemStyle} onClick={onClick}>
    <ProToolbarIcon icon={icon} />
    <span style={proToolbarLabelStyle}>{label}</span>
  </div>
);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'var(--bg-main)', overflow: 'hidden', width: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <ProToolbarItem label="Perfiles" icon={UserRound} onClick={() => alert('Abriendo perfiles...')} />
          <ProToolbarItem label="Actualizar Perfil" icon={RefreshCw} onClick={() => alert('Actualizando perfil de usuario...')} />
          <ProToolbarItem label="Copia de seguridad" icon={Database} onClick={() => alert('Copia de seguridad en progreso...')} />
          <ProToolbarItem label="Optimizar base" icon={Settings} onClick={() => alert('Optimizando base de datos...')} />
      ...
      <div style={{ display: 'flex', flexGrow: 1, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              { name: 'Costos Unitarios', color: '#7c3aed', icon: CircleDollarSign },
              { name: 'Grupos de Costos', color: '#f97316', icon: FolderOpen },
              { name: 'Unidades', color: '#475569', icon: Ruler },
              { name: 'Lista de Insumos', color: '#0ea5e9', icon: ClipboardList },
              { name: 'Diccionario', color: '#b45309', icon: BookOpen },
              { name: 'Indices', color: '#15803d', icon: ChartNoAxesCombined },
              { name: 'Proveedores', color: '#ca8a04', icon: UserRound },
              { name: 'Clientes', color: '#ca8a04', icon: Users },
              { name: 'Tipos de Obra', color: '#991b1b', icon: HardHat },
              { name: 'Encabezados', color: '#6b21a8', icon: Type },
              { name: 'Plantillas GsGs', color: '#be123c', icon: FileText },
              { name: 'Parametros', color: '#334155', icon: SlidersHorizontal }
            ].map(opt => {
              ...
              return (
                ...
                >
                  <span style={{ width: '28px', height: '28px', borderRadius: '7px', background: 'rgba(255,255,255,0.16)', color: '#ffffff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                    <OptionIcon size={18} strokeWidth={2.45} />
                  </span>
